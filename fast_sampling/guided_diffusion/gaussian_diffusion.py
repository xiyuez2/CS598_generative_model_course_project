...
#         print("self.sqrt_recipm1_alphas_cumprod: ", self.sqrt_recipm1_alphas_cumprod)
    ...
    def p_sample(
        ...
        # print(t, nonzero_mask)
        # add self consistency here:
        model_mean = out["mean"].unsqueeze(0)
        # print("model_mean:", model_mean.shape)
        condition_tensors = model.condition
        ...
        if len(self.self_consistency_config) > 0:
            ...
            for target_ind in self.self_consistency_config.keys():
                ...
                if downsample == "pool":
                    ...
                    target_tensor_pool = th.nn.functional.avg_pool3d(target_tensor.unsqueeze(1), int(downsampling_factor), int(downsampling_factor))
                    target_tensor_pool = th.repeat_interleave(target_tensor_pool, int(downsampling_factor), dim=2)
                    target_tensor_pool = th.repeat_interleave(target_tensor_pool, int(downsampling_factor), dim=3)
                    target_tensor_pool = th.repeat_interleave(target_tensor_pool, int(downsampling_factor), dim=4)
        ...
        sample = model_mean[0] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
